...
{
    private static KeyValuePair<string, int> Entry(string fish, int extra) => new(fish, extra);
    ...
    public void KeepsOnlyTheSpeciesThatCarryABonus()
    {
        ...
        var table = BonusRules.BuildSpeciesTable(new[]
        {
            Entry("$item_fish1", 0),
            Entry("$item_fish3", 1),
            Entry("$item_fish9", 2),
    ...
    public void HandlesNoInputWithoutThrowing()
    {
        ...
        Assert.Empty(BonusRules.BuildSpeciesTable(new KeyValuePair<string, int>[0]));
    }
